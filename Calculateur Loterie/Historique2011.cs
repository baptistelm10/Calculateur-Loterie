...
{
    class Historique2011
    {
        public void RecuperationHistorique()
        {
            Console.WriteLine("###### HISTORIQUE 2011 ######");

            var html = @"https://www.euro-millions.com/fr/archive-resultats-2011";
            HtmlWeb web = new HtmlWeb();
            var htmlDoc = web.Load(html);

            var node1 = htmlDoc.DocumentNode.SelectNodes("//ul[@class='balls']");

            using (StreamWriter sw = new StreamWriter(@"C:\Historique\2011.txt", true))
            {
                if (node1 != null)
                {
