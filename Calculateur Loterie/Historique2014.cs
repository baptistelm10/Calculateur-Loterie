...
{
    class Historique2014
    {
        public void RecuperationHistorique()
        {
            Console.WriteLine("###### HISTORIQUE 2014 ######");

            var html = @"https://www.euro-millions.com/fr/archive-resultats-2014";
            HtmlWeb web = new HtmlWeb();
            var htmlDoc = web.Load(html);

            var node1 = htmlDoc.DocumentNode.SelectNodes("//ul[@class='balls']");

            using (StreamWriter sw = new StreamWriter(@"C:\Historique\2014.txt", true))
            {
                if (node1 != null)
                {
