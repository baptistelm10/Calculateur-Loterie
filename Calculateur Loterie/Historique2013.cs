...
{
    class Historique2013
    {
        public void RecuperationHistorique()
        {
            Console.WriteLine("###### HISTORIQUE 2013 ######");

            var html = @"https://www.euro-millions.com/fr/archive-resultats-2013";
            HtmlWeb web = new HtmlWeb();
            var htmlDoc = web.Load(html);

            var node1 = htmlDoc.DocumentNode.SelectNodes("//ul[@class='balls']");

            using (StreamWriter sw = new StreamWriter(@"C:\Historique\2013.txt", true))
            {
                if (node1 != null)
                {
